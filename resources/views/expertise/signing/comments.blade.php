      ...
      <input type="hidden" id="approver_id1" name="approver_id1">  
        ...
        @endif
          ...
          @if (auth()->user()->hasRole('ROLE_KIB_EXPERTISE_EXECUTOR'))
            <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
            <button type="submit" name="" id="" class="btn" style="font-size: 14px; background: #b90404;">Отправить ГТС</a>
          @endif
...
<div class="buttons-wrapper" style="padding-top: 25px;">
  {{-- <a class="btn btn_white" style="font-size: 14px;" onclick="history.back()">← Вернуться назад</a> --}}
    ...
    <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
    <button type="submit" name="discart_gos" id="discart_gos" class="btn" style="font-size: 14px; background: #b90404;">Вернуть</a>
    @endif
...
<div class="buttons-wrapper">
   ...
   @if(auth()->user()->hasRole('ROLE_GO_EXPERTISE_SIGNER') && $expertise->send_to_go_signer == 1)
    ...
    @if (auth()->user()->hasRole('ROLE_GO_EXPERTISE_CONFIRMER') && $expertise->send == 1)
        <button id='approve_go' name="approve_go" onclick="approveGo({{$expertise->id}})" style="display:block; background: #00317B; font-size: 5px;">Согласовать</button>
    @endif

  @if (auth()->user()->hasRole('ROLE_UO_EXPERTISE_DANA') && $expertise->accept_uo_reviewer == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="sendToConfirmers()" style="font-size: 14px; background: #00317B;">Отправить</button>
  @endif  
  ...
  @if (auth()->user()->hasRole('ROLE_UO_EXPERTISE_REVIEWER') && $expertise->send_to_uo_reviewer == 1) 
    ...
    <button class="btn" onclick="acceptUoReviewer()" style="font-size: 14px; background: #00317B;">Принять</button>
  @endif
  ...
  @if(auth()->user()->hasRole('ROLE_GTS_EXPERTISE_CONFIRMER') && $expertise->send_to_gts == 1 && $expertise->accept_gts_reviewers == 0)
      <button id="add-approver-btn">Добавить исполнителя</button>
  @endif
...
document.getElementById("showCommentsButton").addEventListener("click", function() {
    window.location.href = "expertise.signing.comments_show.blade.php";
});
</script>
