...
<div class="buttons-wrapper">
  ...
  @if(auth()->user()->hasRole('ROLE_GO_EXPERTISE_SIGNER') && $expertise->send_to_go_signer == 1)
    ...
    <button  class="btn" id='sign_ecp_button' name="sign_ecp_button" onclick="acceptPassportGO()" style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
  @endif

  @if (auth()->user()->hasRole('ROLE_SI_EXPERTISE_REVIEWER') && $expertise->send_to_si_reviewers == 1)
    <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
    <button class="btn" id='sign_si_ecp_button' name="sign_si_ecp_button" onclick="acceptPassportSI()" style="display: block; font-size: 14px; background: #0178BC;">Подписать Эцп</button>
  @endif
  ...
  @if (auth()->user()->hasRole('ROLE_SI_EXPERTISE_CONFIRMER') && $expertise->accept_si_reviewers == 1) 
    @if (auth()->user()->id == 2669 && $expertise->accept_si_confirmer_first == 0)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" id='sign_siCon_ecp_button' name="sign_siCon_ecp_button" onclick="acceptSIConfirmerFirst()" style="display: block; font-size: 14px; background: #0178BC;">Подписать Эцп</button>
    @endif

    @if (auth()->user()->id == 15 && $expertise->accept_si_confirmer_second == 0)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" id='sign_siCon_ecp_button' name="sign_siCon_ecp_button" onclick="acceptSIConfirmerSecond()" style="display: block; font-size: 14px; background: #0178BC;">Подписать Эцп</button>
    @endif
  @endif


  @if(auth()->user()->hasRole('ROLE_GTS_EXPERTISE_CONFIRMER') && $expertise->send_to_gts == 1 && $expertise->accept_gts_reviewers == 0)
      <button class='btn' id="add-approver-btn">Добавить исполнителя</button>
  @endif

  @if(auth()->user()->hasRole('ROLE_GTS_EXPERTISE_REVIEWER'))
      <button class='btn' id="add-app-btn" style="display: none">Добавить Согласующего</button>
      <button class='btn' id="add-signer-btn" style="display: none">Добавить Подписанта</button>
  @endif

  @if(auth()->user()->hasRole('ROLE_UO_EXPERTISE_REVIEWER'))
    @if($expertise->send_to_uo_si == 1 && $expertise->send_to_uo_gts == 1)
  ...
  @if (auth()->user()->hasRole('ROLE_GTS_EXPERTISE_REVIEWER'))
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="acceptGTSReviewer()" id="gts_reviewer"  style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
  @endif 

  @if (auth()->user()->hasRole('ROLE_UO_EXPERTISE_REVIEWER'))
    @if($expertise->send_to_uo_si == 1 && $expertise->send_to_uo_gts == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="acceptGTSReviewer()"  style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
    @endif  
  @endif

  @if(auth()->user()->hasRole('ROLE_GTS_EXPERTISE_CONFIRMER') && $expertise->accept_gts_reviewers == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="acceptGtsConfirmer()"  style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
  @endif

  @if(auth()->user()->hasRole('ROLE_SI_EXPERTISE_SIGNER') && $expertise->send_to_si_signer == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      @if(empty($expertise->ecp_si_signer))
      <button class="btn" onclick="acceptSiSigner()" id="accept_si_signer"  style="font-size: 14px; background: #00317B; display: block;">Подписать Эцп</button>
      @endif
  @endif

  @if(auth()->user()->hasRole('ROLE_GTS_EXPERTISE_SIGNER') && $expertise->send_to_gts_signer == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="acceptGTSSigner()"  style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
  @endif

  @if(auth()->user()->hasRole('ROLE_UO_EXPERTISE_CONFIRMER') && $expertise->send_to_uo_confirmer == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="acceptUOConfirmer()"  style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
      <button class="btn" onclick="confirmUo()"  style="font-size: 14px; background: #00317B;">Согласовать</button>
  @endif

  @if(auth()->user()->hasRole('ROLE_UO_EXPERTISE_SIGNER') && $expertise->accept_uo_to_confirm == 1)
      <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
      <button class="btn" onclick="acceptUOSigner()"  style="font-size: 14px; background: #00317B;">Подписать Эцп</button>
  @endif
...
<div> 
  @if ($expertise->accept_uo_reviewer == 1)
    ...
    <div class="is-checkboxes" style="margin-bottom: 15px;">
      <label class="toggle-checkbox">
...
<div class="buttons-wrapper">
  @if ($expertise->accept_uo_reviewer == 0)
  <input type="hidden" id="send_to_mcriap_executor" name="send_to_mcriap_executor">
  <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
  <div style="display: flex; flex-direction: column; width: 100%;">
    <label for="send_to_mcriap_executor" style="width: 50%; margin-bottom: 10px;">Выбрать исполнителя:</label>
    <select id="send_to_mcriap_executor" name="send_to_mcriap_executor" style="width: 20%; padding: 8px 12px;">
...
@if ($expertise->accept_uo_reviewer == 1)
<input type="hidden" id="add_to_mcriap_executor" name="add_to_mcriap_executor">
<input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
<div style="display: flex; flex-direction: column; width: 100%; margin-top: 10px;">
  <label for="add_to_mcriap_executor" style="width: 20%; margin-bottom: 10px;">Добавить исполнителя:</label>
  <select id="add_to_mcriap_executor" name="add_to_mcriap_executor" style="width: 20%; padding: 8px 12px;">
    <option value="" selected disabled>Выбрать исполнителя</option>
    @foreach($users as $user)
      @if($user->hasRole('ROLE_UO_EXPERTISE_REVIEWER'))
        <option value="{{ $user->id }}">{{ $user->name }} {{ $user->surname }} {{ $user->middlename }}</option>
...
@endif
    ...
    @if ($expertise->send_to_uo == 1 && $expertise->accept_uo_reviewer == 0 ) 
        <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
...
@if (auth()->check() && (auth()->user()->hasRole('ROLE_SI_EXPERTISE_CONFIRMER') || auth()->user()->hasRole('ROLE_KIB_EXPERTISE_CONFIRMER')))
    ...
    <div class="buttons-wrapper">
      ...
      <div style="display: flex; flex-direction: column; width: 100%;">
        <label for="approver1" style="margin-bottom: 10px;">Выбрать исполнителя</label>
        <select id="approver1" name="approver_id1" style="width: 20%; padding: 8px 12px; margin-bottom: 15px;">
        <option selected disabled>Выберите исполнителя</option>
          @foreach($users as $user)
            @if($user->hasRole('ROLE_SI_EXPERTISE_REVIEWER'))
              <option value="{{ $user->id }}">{{ $user->name }} {{ $user->surname }} {{ $user->middlename }}</option>
            @endif
          @endforeach
        </select>
        <button class="btn" type="submit" id="send_to" name="send_to" style="font-size: 14px; background: #0178BC; width: 20%;">Отправить</button>
      </div>
...
<div class="buttons-wrapper">
  ...
  <div style="display: flex; flex-direction: column; width: 100%;">
    <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
    <select id="signer_id" name="signer_id" style="width: 25%; padding: 8px 12px; margin-bottom: 10px; margin-top: 10px;">
    ...
    <button class="btn" type="submit" id="accept_si_reviewer" name="accept_si_reviewer" style="display: none; font-size: 14px; background: #0178BC; width: 20%;">Отправить на согласование</button>
    <button class="btn" type="submit" id="save_si_reviewer" name="save_si_reviewer" style="display: block; font-size: 14px; background: #0178BC; margin-top: 20px; width: 20%;">Сохранить заключения</button>
  </div>
@endif
  ...
  @if(auth()->user()->hasRole('ROLE_GTS_EXPERTISE_CONFIRMER') && $expertise->accept_gts_reviewers == 1)
    <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
    <button class="btn"  id="accept_gts_confirmer" name="accept_gts_confirmer" style="font-size: 14px; background: #00317B; display:none;">Согласовать</button>
  @endif
  ...
  @if (auth()->user()->hasRole('ROLE_GTS_EXPERTISE_REVIEWER'))
    ...
    <button class="btn"  id="accept_gts_reviewer" name="accept_gts_reviewer" style="font-size: 14px; background: #00317B; display:none">Отправить на согласование</button>
  @endif
    ...
    @if (auth()->user()->hasRole('ROLE_UO_EXPERTISE_REVIEWER') && $expertise->send_to_uo_reviewer == 1) 
    <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
    ...
    @if (auth()->user()->hasRole('ROLE_KIB_EXPERTISE_EXECUTOR'))
        <input type="hidden" name="expertise_id" id="expertise_id" value="{{ $expertise->id }}">
    ...
    @if(auth()->user()->hasRole('ROLE_SI_EXPERTISE_SIGNER') && $expertise->send_to_si_signer == 1)
      ...
      @if(!empty($expertise->ecp_si_signer))
      <button class="btn" id="send_to_uo_si" name="send_to_uo_si"  style="font-size: 14px; background: #00317B; display: block;">Отправить в УО</button>
      @endif
    @endif
...
@if(auth()->user()->hasRole('ROLE_GO_EXPERTISE_SIGNER') || auth()->user()->hasRole('ROLE_GO_EXPERTISE_EDITOR') || auth()->user()->hasRole('ROLE_GO_EXPERTISE_CONFIRMER'))
  <h2 class="is-content-title">При необходимости укажите комментарий</h2>
  <textarea class="form__field" style="overflow-y: scroll !important; min-height: 300px;" id="comment" name="comment">{{ $expertise->comment_discart }} {{ $expertise->comment_accept }}</textarea>
@endif
...
@include('expertise.info.comments_scripts')
