    ...
    <tr>
      ...
      <td style="width: 70%; padding: 12px 20px 12px 20px;">
        <select id="selected_is_for_change" name="selected_is_for_change" required>
          ...
          @foreach ($iss as $is)
          <option value="{{ $is->id }}" @if($expertise->selected_is_for_change == $is->id) selected @endif>{{ $is->$names }}</option>
          @endforeach
        </select>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td style="width: 70%; padding: 12px 20px 12px 20px;">
        <select id="selected_is_for_exit" name="selected_is_for_exit" required>
          ...
          @foreach ($iss as $is)
          <option value="{{ $is->id }}" @if($expertise->selected_is_for_exit == $is->id) selected @endif>{{ $is->$names }}</option>
          @endforeach
        </select>
      </td>
    </tr>
...
<div class="is-menu-content" data-id="1" style="display: block;">
  <table class="is-table">
    <tr>
      ...
      <td></td>
      <td style="width: 70%; padding: 12px 20px 12px 20px;"><input value="{{ $expertise->paln_integrations }}" id="paln_integrations" name="paln_integrations" type="text" minlength="2" maxlength="500" autofocus></td>
    </tr>
  </table>
</div>