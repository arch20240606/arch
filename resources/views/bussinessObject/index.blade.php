...
<main class="content">
  ...
  <div class="container">
    ...
    <div class="breadcrumbs">
      ...
      /
      <a href="{{ route('bussinessObject.index') }}">Объекты данных</a>
    </div>
  ...
  <button class="tabs__button btn" style="border-radius: 8px 8px 8px 8px;" type="submit">Найти</button>
    ...
    @if ($iss->count() > 0 )
      ...
      <table class="table table_expertise" style="font-weight: normal; color: #000000;">
        ...
        @foreach ($iss as $is)
          <tr>
            <td class="table__status">{{ $is->idBis }}</td>
            {{-- <td class="table__status">{{ $is->_id }}</td> --}}
            {{-- <td class="table__status" style="text-align: left;"><a href="{{ route('accounting.show', ['accounting' => $is->idis]) }}">{{ $is->name }}</a></td> --}}
            <td class="table__status" style="text-align: left;"><a href="{{ route('bussinessObject.show', ['bussinessObject' => $is->idBis]) }}">{{ $is->name }}</a></td>
