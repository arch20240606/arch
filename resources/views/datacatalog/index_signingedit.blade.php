...
<main class="content">
  ...
  <div class="container">
    ...
    <h1 class="page-title">{{ trans('app.catalog') }}</h1> Утверждение паспорта данных
    ...
    <div class="is-info">
      ...
      <form class="form" method="POST" action="{{ route('datacatalog.update', ['datacatalog' => $passport->id]) }}">
        ...
        <div class="buttons-wrapper" style="border-top: 1px solid #e3e5ec; padding-top: 25px; margin-top: 50px;">
          ...
          <button class="btn" type="submit" id="passport_signin_accept" name="passport_signin_accept" style="font-size: 14px; background: #00317B;">{{ trans('app.tab_approve_passport') }}</button>
          <button class="btn" type="submit" id="passport_signin_discart" name="passport_signin_discart" style="font-size: 14px; background: #bc2601;">{{ trans('app.tab_send_finalized') }}</button>
        </div>
